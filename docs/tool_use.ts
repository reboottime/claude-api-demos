...
export async function POST(request: NextRequest) {
  ...
  // Helper to avoid duplicating API call config
  const callClaude = () =>
  ...
  // - stop_reason="tool_use" → Claude requested a tool, loop continues
  ...
  while (response.stop_reason === "tool_use") {
    ...
    steps.push({ type: "tool_call", name, input, content: `Calling ${name}...` });

    // Execute the tool and get result
    const toolResult = await getWeather((input as { location: string }).location);
    steps.push({ type: "tool_result", name, content: toolResult });

    // Add to conversation history:
    // 1. Claude's tool request (as assistant message)
    // 2. Tool result (as user message with tool_result type)
    messages.push(
      { role: "assistant", content: response.content },
      { role: "user", content: [{ type: "tool_result", tool_use_id: id, content: toolResult }] }
    );

    // Next API call: Claude processes tool result and decides next action
